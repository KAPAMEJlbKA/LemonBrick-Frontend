<template>
  <q-dialog v-model="show" @before-show="fetchNewCode()">
    <q-card style="border-radius: 20px; display: flex; flex-direction: column;">
      ...
      <q-card-section v-if="key != null" style="margin-left: auto; margin-right: auto">
        <p style="max-width: 550px">Отсканируйте qr код с помощью мобильного приложения <a href="https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2">Google Authenticator</a>, <a href="https://play.google.com/store/apps/details?id=com.authy.authy">Authy</a> или другого приложения с поддержкой генерации TOTP кодов</p>
        <div class="flex flex-center">
          <qrcode-vue :value="value" :size=300 level="H" />
        </div>
        <p style="word-wrap: break-word; max-width: 520px">Или введите код вручную: <b>{{ key }}</b></p>
      </q-card-section>
      <q-card-section v-if="key == null" style="margin-left: auto; margin-right: auto">
        Выполняется запрос...
      </q-card-section>
      ...
      <q-card-actions style="margin-left: auto; margin-right: auto">
        <q-btn flat color="orange" @click="enable2FA">Включить</q-btn>
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
